...
int main(int argc, char* argv[]){
  ...
  std::map<Link, double> omega;
  std::map<Link, double> alpha;
  std::map<int, int> NM2EO;

  {
    ...
    while (std::getline(file, str)){
      ...
      omega.insert( std::make_pair( Link{i,j}, v ) );
      omega.insert( std::make_pair( Link{j,i}, -v ) );
    }
...

  {
    ...
    while (std::getline(file, str)){
      ...
      alpha.insert( std::make_pair( Link{i,j}, v ) );
...

  {
    ...
    for(auto elem : omega){
      ...
      std::cout << "ix,iy = " << ix << ", " << iy << ", diff = " << (alpha2 + M_PI + omega12) - alpha1 << std::endl;
    }
  }

  {
    NM2EO.insert( std::make_pair(10, 0) );
    NM2EO.insert( std::make_pair( 3, 3) );
    NM2EO.insert( std::make_pair( 9, 5) );
    NM2EO.insert( std::make_pair( 1, 8) );
    NM2EO.insert( std::make_pair( 7, 9) );
    NM2EO.insert( std::make_pair( 8,11) );
    NM2EO.insert( std::make_pair( 6, 7) );
    NM2EO.insert( std::make_pair( 5,10) );
    NM2EO.insert( std::make_pair( 2, 1) );
    NM2EO.insert( std::make_pair(12, 2) );
    NM2EO.insert( std::make_pair( 4, 4) );
    NM2EO.insert( std::make_pair(11, 6) );
  }
  ...
  // std::map<std::pair<int, int>, double> omegaEO;
  // std::map<std::pair<int, int>, double> alphaEO;
  std::map<Link, double> omegaEO;
  std::map<Link, double> alphaEO;
  {
    for(auto elem : alpha){
      ...
      alphaEO.insert( std::make_pair( Link{ix2,iy2},
				      alpha[Link{ix1,iy1}] ) );
    }
    ...
    for(auto elem : omega){
      ...
      omegaEO.insert( std::make_pair( Link{ix2,iy2},
				      omega[Link{ix1,iy1}] ) );
    }
  ...
  // NM 10,3
  {
    // for(auto elem : omega){
    {
      ...
      std::cout << "diff NM = " << (alpha2 + M_PI + omega12) - alpha1 << std::endl;
    }

    {
      ...
      double alpha1 = alphaEO[Link{ix,iy}];
      double alpha2 = alphaEO[Link{iy,ix}];
      double omega12 = omegaEO[Link{ix,iy}];

      std::cout << "diff EO = " << (alpha2 + M_PI + omega12) - alpha1 << std::endl;
    }
  }
