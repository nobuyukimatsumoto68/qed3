...
int main(int argc, char* argv[]){
  ...
  for( auto& sol : sols){
    ...
    if(!sol.is_split){
      ...
      assert( std::abs(cosl(alpha0)-e0[0]) + std::abs(std::sin(alpha0)-e0[1]) < TOLLOOSE );
      ...
      assert( std::abs(cosl(alpha1)-e1[0]) + std::abs(std::sin(alpha1)-e1[1]) < TOLLOOSE );
    }
    else{
  ...
  for(Idx ix=0; ix<sites.size(); ix++){
    const V3 e0x = Pt( sites[ix] ).ehat0();
    const V3 e1x = Pt( sites[ix] ).ehat1();
    for(Idx iy : nns[ix]){
  ...
  // u, v
  {
    ...
    for(Idx ix=0; ix<nns.size(); ix++){
      ...
      VD e1, e2, e3;
      e1 << cosl(al1), std::sin(al1);
      e2 << cosl(al2), std::sin(al2);
      e3 << cosl(al3), std::sin(al3);
