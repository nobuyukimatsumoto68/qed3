...
struct MatPoly{
  ...
  MatPoly( const std::initializer_list<int> struc={1} )
    : vec_mats(struc.size())
    , coeffs(struc.size())
  {
  ...
  void push_back( const T coeff, const std::initializer_list<SparseMatrix<T>*> struc={} ){
    coeffs.push_back(coeff);
    vec_mats.push_back(std::vector<SparseMatrix<T>*>(0));
    vec_mats.back().resize( struc.size() );
