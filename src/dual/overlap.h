...
struct OverlapPseudoFermion {
  ...
  //   // double operator()( const double x ) const {
  //   /// double res = 2.0 / (1.0+sgn.lambda_inv) * x / (sgn.k*sgn.M);
  //   for(int m=1; m<sgn.size; m++) {
  //     res *= (sgn.k*sgn.k - sgn.c[m]*x*x) / (sgn.k*sgn.k - sgn.cp[m]*x*x);
  //   }

  //   double coeff = 2.0 / (1.0+sgn.lambda_inv) / (sgn.k*sgn.M);
  //   for(Idx i=0; i<sparse.N; i++) res[i] = 
