...
struct OverlapWMass : public Zolotarev {
  ...
  bool is_lambda_fixed = false;   // _claude: freeze lambda_max after first update() (HMC-exactness fix, 2026-07-12)
  ...
  // _claude (HMC-exactness fix, 2026-07-12 per NM): driver calls this after thermalization to freeze
  // lambda_max. FREE (recompute) while thermalizing, then held fixed -> exact. (Per-step recompute made
  // D_ov change step-to-step while the force omits d lambda_max/dU -> a step-independent dH floor.)
