...
# "c += rescaled mass" scheme, from the TUNING sandbox (tuning/includes copy + tuning driver). Tests the L1
# ladder {0,1.0} shifted per physical mass m in {0.1,0.5,1.0,1.5} (frame0 = phys mass via set_mass; aux =
# 1.0 + resc, resc = m*Abar/abar_s). Validates get_force() == +dS/dU (central FD, |grad-fd| < 1e-4).
# GRAD_L4 = the production block grad. No rm.
set -u
...
{
  echo "######## MASSIVE Hasenbusch force gate (tuning)  $(date)  GPU=${GPU} ########"
