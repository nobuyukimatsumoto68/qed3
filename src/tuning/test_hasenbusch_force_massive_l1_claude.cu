...
// _claude (MASSIVE, c += rescaled mass): pass the MASSLESS ladder0 {0, c_1, ...} + the physical mass;
// run_ladder sets frame0 = phys mass and shifts aux coeffs by resc = phys_mass*Abar/abar_s (after base).
static bool run_ladder( const std::vector<Complex>& ladder0, const Complex phys_mass, const std::string& tag );
...
int main(int argc, char* argv[]){
  ...
  // MASSIVE force test: the production L1 ladder {0, 1.0} shifted by "c += rescaled mass" for each physical
  // mass. Frame0 = phys mass (set_mass, rescaled internally); aux = 1.0 + resc. Tests the massive force path.
  bool ok = true;
  const std::vector<Complex> ladder0 = { Complex(0.0,0.0), Complex(1.0,0.0) };   // L1 massless ladder
  const double phys_masses[] = { 0.1, 0.5, 1.0, 1.5 };
  for( const double m : phys_masses ){
    ok = run_ladder( ladder0, Complex(m,0.0), "L1_m"+std::to_string(m) ) && ok;
  }

  std::cout << "\n# ===== MASSIVE Hasenbusch force gate (c += rescaled mass): " << (ok ? "PASS" : "FAIL")
            << " (heatbath ~1e-5; force grad-vs-FD < 1e-4) =====" << std::endl;
  return ok ? 0 : 1;
}
...
static bool run_ladder( const std::vector<Complex>& ladder0, const Complex phys_mass, const std::string& tag ){
  ...
  // _claude (c += rescaled mass): frame0 = physical mass; aux coeffs shifted UP by resc = m*Abar/abar_s
  // so the additive inter-frame gaps (massless-tuned) are preserved. Matches the HMC driver's shift.
  std::vector<Complex> masses = ladder0;
  masses[0] = phys_mass;
  const Complex resc = phys_mass * (base.mean_dual_area / base.mean_ell);
  for(size_t i=1; i<masses.size(); i++) masses[i] += resc;
  std::cout << "# MASSIVE ladder: phys m=" << phys_mass.real() << " resc=" << resc.real()
            << " -> {frame0(set_mass)=" << masses[0].real() << ", aux(set_mass_coeff)=" << masses[1].real() << "}" << std::endl;
