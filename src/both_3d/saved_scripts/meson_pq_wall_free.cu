...
int main(int argc, char* argv[]){
  ...
  // ----------------------
  const double at = 0.05; // T/Comp::Nt;
      ...
      for(int ell=0; ell<=ellmax; ell++){
        for(int em=-ell; em<=ell; em++){
          for(int ab=0; ab<=3; ab++){
            ...
            const std::string path=dir4+"meson_"+std::to_string(ell)+"_"+std::to_string(em)+"_a"+std::to_string(ab)+"_h"+std::to_string(h)+"_corr_v2.dat";
            std::ofstream ofs(path);
