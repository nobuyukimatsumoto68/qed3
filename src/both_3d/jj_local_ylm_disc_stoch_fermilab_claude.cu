...
// _fermilab copy: geometry path HARDCODED (absolute), as in hmc_fermilab_wmass*.cu:74, so the
// binary runs from any cwd (e.g. a lustre output dir) -- no relative "../../geometry" dependency.
// const std::string dir = "../../geometry/data/";   // original (relative; needs cwd=src/both_3d)
