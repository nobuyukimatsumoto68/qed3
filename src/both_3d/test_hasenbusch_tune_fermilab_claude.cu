...
// _fermilab: ABSOLUTE geometry (FNAL) -- run CWD is the /lustre2 output dir, relative "../../geometry" breaks.
...
int main(int argc, char* argv[]){
  ...
  std::string cfgdir = "Nf"+std::to_string(Nf)+"_gsq"+std::to_string(gsq)+"at"+std::to_string(at)
                     + "nu0"+std::to_string(nu0)+"nt"+std::to_string(Comp::Nt)
                     + "L"+std::to_string(Comp::N_REFINE)+"/";
  // _fermilab: the redo dir3 = head + "mRe0.000000mIm0.000000nt128L<L>_hb<tag>/". Fall back to that naming,
  // then glob the L<L>_hb* dir (hb1.000000 for L1/L2, hb0.400000-1.000000 for the 3-stage L3/L4).
  if(!std::filesystem::exists(cfgdir)){
    ...
    const std::string alt = head + "mRe0.000000mIm0.000000nt"+std::to_string(Comp::Nt)
                          + "L"+std::to_string(Comp::N_REFINE)+"/";
    if(std::filesystem::exists(alt)) cfgdir = alt;
    else for(const auto& e : std::filesystem::directory_iterator(".")){
      if(!e.is_directory()) continue;
      const std::string nm = e.path().filename().string();
      if(nm.rfind(head,0)==0 && nm.find("L"+std::to_string(Comp::N_REFINE)+"_hb")!=std::string::npos){ cfgdir = nm+"/"; break; }
    }
  }
  std::vector<int> ks;
  ...
  const int MG = 100;   // _fermilab L3: gauge substeps = production hasenbusch_mg(3)=100 (apples-to-apples with the run)
  // L3 Hasenbusch scheme COMPARISON (2026-07-27): 3-stage (committed) vs 2-stage. Lower Osborn Cost wins.
  std::vector<Setup> setups = {
    { "3st {0,0.4,1.0}/{3,3,3}", { Complex(0.0,0.0), Complex(0.4,0.0), Complex(1.0,0.0) }, { 3, 3, 3 }, MG },  // committed L3
    { "2st {0,1.0}/{4,4}",       { Complex(0.0,0.0), Complex(1.0,0.0) },                   { 4, 4 },    MG },  // alt 2-stage
  };
