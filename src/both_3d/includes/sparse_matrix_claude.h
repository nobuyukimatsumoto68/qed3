...
// sparse_matrix_claude.h (C4b): copy of sparse_matrix.h whose LinOpDHDWrapper routes
// D^dag D through the multi-shift DHD_deviceAsyncLaunch_ms. Include THIS (instead of
// sparse_matrix.h) in a .cu to make its overlap solves use multi-shift. Only one of the
// two must be included per translation unit (identical type definitions otherwise clash).
...
struct LinOpDHDWrapper : public LinOp {
  ...
  void operator()( T* d_res, const T* d_v ) const {
    ...
    // D.DHD_deviceAsyncLaunch( d_res, d_v );      // original (pole loop)
    D.DHD_deviceAsyncLaunch_ms( d_res, d_v );      // C4b: multi-shift inner solve
