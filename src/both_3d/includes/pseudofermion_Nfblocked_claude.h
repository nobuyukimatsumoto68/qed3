...
// pseudofermion_Nfblocked_claude.h -- Nf-flavor BLOCK (mrhs) pseudo-fermion manager.
...
#ifdef BLOCK_FORCE
  BlockedForce<N,NSTACK,Fermion> bforce; // Nf-block force (Phase 2); borrows blk's pool scratch
#endif
...
#ifdef BLOCK_FORCE
  // total fermion force over all NSTACK flavors -> dSf (single gauge-momentum force). Uses the current
  // d_eta_blk (call update_eta first). Phase-2 block form of the serial precalc_grad + grad_l4 loop.
  template<typename Force, typename Gauge>
  void compute_force( Force& dSf, const Gauge& U ) { bforce.compute( dSf, U, d_eta_blk ); }
#endif
