// jj_disc_block_t_claude.cu  (C6f-c: t-BLOCKED sink variant of jj_disc_claude.cu)
// Same physics/output as jj_disc; the four sink loops (all already outer-n/a inner-t, all forward K) are
// block-t: `for t: op_K.from_cpu(kphi,sinkvec)` -> `apply_k_block_t(d_kphi_block,sinkvec,U,fixed); D2H;
// for t: <same accumulation on column t>`. Disc ylm is a SCALAR sum so NO PhiL tower restructure (unlike
// jj_corr_block_t). All loops dag=false => only apply_k_block_t. Numerically == jj_disc to ~1e-15 (block
// K's Term B solve_shift_block) -> validate h5diff -d 1e-10. Sink K applies drop Nt*n -> n block applies.
//
...
#include "blocked_mat_claude.h"          // BlockedMat + BlockMemPool + solve_shift_block (block kernels via matpoly_claude.h)
#include "conserved_current_claude.h"   // ConservedCurrent: apply_k / apply_k_dag (multishift apply_k_ms)
#include "conserved_current_block_claude.h"  // C6f-c: ConservedCurrentBlockT::apply_k_block_t (t-blocked sink)
...
int main(int argc, char* argv[]){
  ...
  // C6f-c: t-blocked sink kernel.  apply_k_block_t computes K(t,fixed) sinkvec for ALL t in one pass.
  ...
  // C6f-c sink block-t buffers: d_sinkvec = sink vector (phi'/tilphi) uploaded once per loop; d_kphi_block
  // = K(t,fixed) sinkvec for all t (N*Nt device); kblk = its host copy; kcol(t) copies column t into
  // kphi.field so the existing per-(n,t) accumulation lines run UNCHANGED.
  ...
  for(int k = 0; k <= kmax; k += k_ckpoint){
    ...
    for(int h=0; h<nhits; h++){
      ...
      CUDA_CHECK(cudaMemcpy(d_sinkvec, reinterpret_cast<CuC*>(phi.field), N*CD, H2D));  // sink = phi' (tp + sp)
      for(int n=0; n<n_sites; n++){
      ...
      for(int a=0; a<n_links; a++){
        ...
        kblock.apply_k_block_t(d_kphi_block, d_sinkvec, U, lk);       // K(lk,t) phi' for ALL t (d_sinkvec still phi')
        CUDA_CHECK(cudaMemcpy(reinterpret_cast<CuC*>(kblk.data()), d_kphi_block, (size_t)N*Nt*CD, D2H));
        for(int t=0; t<Nt; t++){
      ...
      if(parity){
        ...
        CUDA_CHECK(cudaMemcpy(d_sinkvec, reinterpret_cast<CuC*>(tilphi.field), N*CD, H2D));  // sink = tilphi (tp + sp)
        for(int n=0; n<n_sites; n++){
        ...
        for(int a=0; a<n_links; a++){
          const BaseLink lk = base.links[a];
          const Idx il = base.map2il.at(lk);
          kblock.apply_k_block_t(d_kphi_block, d_sinkvec, U, lk);      // K(lk,t) tilphi for ALL t (d_sinkvec still tilphi)
          CUDA_CHECK(cudaMemcpy(reinterpret_cast<CuC*>(kblk.data()), d_kphi_block, (size_t)N*Nt*CD, D2H));
          for(int t=0; t<Nt; t++){
