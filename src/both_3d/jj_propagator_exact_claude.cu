...
// propagator -- for the contraction stage (jj_contract_exact_claude.cu).  No stochastic estimator.
//
...
// field (single config k=1, U=1; the conformal test).  Output: data_<ESNID>/prop_exact_L<L>/
...
int main(int argc, char* argv[]){
  ...
  // ---- output dir: data_<ESNID>/prop_exact_L<L>/  ----
  ...
  const std::string dir_out = "data_"+esnid+"/prop_exact_L"+std::to_string(Comp::N_REFINE)+"/";
  ...
  auto write_cvec = [](HighFive::File& h5, const std::string& key, const std::vector<Complex>& C){
    ...
    for(size_t kk=0;kk<C.size();kk++){ re[kk]=C[kk].real(); im[kk]=C[kk].imag(); }
    h5.createDataSet(key+"/real", re);  h5.createDataSet(key+"/imag", im);
  };
  // write a dense inverse as ROW-MAJOR P[row*N+col] = (M^{-1})_{row,col} (re/imag flat, length N^2).
  // d_A0 (column-major D_ov) + diagonal shift c -> M; LU-solve M X = I -> X = M^{-1}; transpose to row-major.
  auto invert_shift = [&](const Complex& c)->std::pair<std::vector<double>,std::vector<double>>{
    std::vector<CuC> M(A0, A0+(size_t)n*n);             // copy dense D_ov
    for(int i=0;i<n;i++){ CuC& d=M[(size_t)n*i+i];
    ...
    for(int col=0;col<n;col++) for(int row=0;row<n;row++){
      ...
      re[(size_t)n*row+col] = cuCreal(v);  im[(size_t)n*row+col] = cuCimag(v);  // -> row-major
    }
  ...
  for(int k=0; k<=kmax; k+=k_ckpoint){
    ...
    auto Dm = invert_shift(mP);
    std::pair<std::vector<double>,std::vector<double>> Dt;
    if(parity) Dt = invert_shift(mtil);
    std::cout << "#   inverted ["<<timer.currentSeconds()<<" s]" << std::endl;

    // ---- write data_<ESNID>/prop_exact_L<L>/Dinv.<k>.h5  (ATOMIC: .tmp + rename) ----
