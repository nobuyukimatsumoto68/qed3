...
// Sec. 3.8).  ONE program computes EVERYTHING per config, sharing phi'=D_m^{-1} eta and the K phi' applies
// between the disconnected and the connected pieces:
//   - disconnected (vector): single-time traces T(a,t) = eta^dag K(a,t) phi'  (apply-only; raw dump).
...
// SAME phi'.  One temporal-site insertion loop feeds disc-tp + conn-tp + conn-ylm; one spatial-link loop
// feeds disc-sp + conn-sp.  Only the connected SOURCE legs cost solves.
//
