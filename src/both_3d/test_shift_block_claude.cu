// test_shift_block_claude.cu
// C6f-b0 (device): validate + BENCHMARK the single-shift block CG (BlockedMat::solve_shift_block,
// solving (A + sigma) X_c = b_c, A = (1/lmax^2) M_DWH M_DW) against nstack single inner pole solves
// (MatPoly::solve over the SAME seed operator A + sigma). This is the Term B inner pole solve of
// conserved_current_claude.h:354; batching its npole independent solves over the Nt-wide t-block is
// the C6f lever. By construction (single-shift block CG == per-column standard CG, per-column freeze,
// shared worst-column stop) block column c equals the single solve on b_c BIT-FOR-BIT.
//   block: blk.solve_shift_block(d_Xblk, d_B, sigma, TOL_INNER)
//   ref:   for each c, op_seed.solve<N>(d_ref, d_B + c*N, TOL_INNER)   (op_seed = A + sigma MatPoly)
// Tested at the seed pole (min sigma => worst conditioned) and a mid pole; NSTACK = 1 / 4 / Nt.
// PASS = max|block-single| < 1e-10. Free field (U=1). Reports wall-time block vs nstack-serial.
//
// Reference (algorithm): standard CG; block batching for occupancy (cf. solve_sq, B. Jegerlehner
// hep-lat/9612014 is the multishift used elsewhere -- NOT here; this is a single explicit shift).
//
// Compile: handled by the both_3d Makefile. Run: ./test_shift_block_claude.o [--mass-re x --mass-im y]
...
#include "blocked_mat_claude.h"   // C7: BlockedMat<N,NSTACK,Op> + solve_shift_block (C6f-b0)
...
int main(int argc, char* argv[]){
  ...
  // pole shifts sigma_m = -k^2/cp[m] (m=1..size-1); seed = smallest sigma (slowest CG = worst stress).
  ...
  const int mid = npole/2;
  std::cout << "# npole="<<npole<<"  seed pole="<<seed<<" (sigma="<<sigma[seed]<<")  mid pole="<<mid
            << " (sigma="<<sigma[mid]<<")" << std::endl;

  std::cout << std::scientific << std::setprecision(3);
  bool all_pass = true;

  auto run = [&]<int NSTACK>(int polem){
    const double sig = sigma[polem];
    // reference seed operator A + sigma = (1/lmax^2){M_DW,M_DWH} + sigma*I  (exactly Term B's solve)
    MatPoly op_seed;
    op_seed.push_back(cplx(1.0/(Dm.lambda_max*Dm.lambda_max)), {&Dm.M_DW, &Dm.M_DWH});
    op_seed.push_back(cplx(sig), {});
    ...
    // ---- block single-shift solve (one call, NSTACK RHS) ----
    CUDA_CHECK(cudaDeviceSynchronize());
    auto t0=std::chrono::steady_clock::now();
    blk.solve_shift_block(d_Xblk, d_B, sig, Comp::TOL_INNER);
    CUDA_CHECK(cudaDeviceSynchronize());
    const double t_blk=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();

    // ---- NSTACK single inner solves (reference + serial timing) ----
    double worst=0.0, t_ref=0.0;
    for(int c=0;c<NSTACK;c++){
      auto tr=std::chrono::steady_clock::now();
      op_seed.solve<N>(d_ref, d_B + (size_t)c*N, Comp::TOL_INNER);
      CUDA_CHECK(cudaDeviceSynchronize());
    ...
    std::cout << "# pole="<<std::setw(2)<<polem<<" nstack="<<std::setw(3)<<NSTACK
              << "  max|block-single|="<<worst<<"  ("<<(ok?"PASS":"FAIL")<<")"
  ...
  for(int pm : {seed, mid}){
    run.operator()<1>(pm);
    run.operator()<4>(pm);
    run.operator()<Comp::Nt>(pm);   // block-t width = Nt (the C6f target)
  }

  std::cout << "# C6f-b0 RESULT: " << (all_pass ? "ALL PASS" : "FAIL") << std::endl;
