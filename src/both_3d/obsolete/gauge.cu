...
namespace Comp{
  ...
  constexpr int NPARALLEL_SORT=10; // 12
...
int main(int argc, char* argv[]){
  ...
  std::string dir="beta"+std::to_string(beta)+"at"+std::to_string(at)+"nt"+std::to_string(Comp::Nt)+"L"+std::to_string(Comp::N_REFINE)+"/";
...
//   // }
  ...
  for(int k=0; k<kmax; k++){
    ...
    if(k%interval==0){
      ...
      // double plaq_mean = 0.0;
      // {
      //   int counter=0;
      //   for(int i_face=0; i_face<base.n_faces; i_face++){
      //     plaq_mean += U.plaquette_angle(s, U.lattice.faces[i_face]);
      //   }
      //   plaq_mean /= counter;
      // }
...
#pragma omp parallel for num_threads(Comp::NPARALLEL_GAUGE)
      ...
      for(int t=0; t<Comp::Nt; t++){
        ...
        for(int s=0; s<Comp::Nt; s++){
          for(int i_face=0; i_face<base.n_faces; i_face++){
            tmp1 += U.plaquette_angle(s, U.lattice.faces[i_face]) * U.plaquette_angle(s+t, U.lattice.faces[i_face]) / std::pow(base.vols[iface],2);
            counter++;
            tmp1 += U.plaquette_angle(s, U.lattice.faces[i_face]) * U.plaquette_angle(s-t, U.lattice.faces[i_face]) / std::pow(base.vols[iface],2);
            counter++;
          }
...
#pragma omp parallel for num_threads(Comp::NPARALLEL_GAUGE)
    ...
    ofs << "# kmax = " << kmax << std::endl;
    for(int t=0; t<Comp::Nt; t++){
      ofs << mean[t] << " " << std::sqrt(var[t]) << std::endl;
