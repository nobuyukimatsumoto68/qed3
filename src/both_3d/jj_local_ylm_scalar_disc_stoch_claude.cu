// jj_local_ylm_scalar_disc_stoch_claude.cu  (copy of jj_local_ylm_disc_stoch_claude.cu + scalar a=0 loop)
...
//
// SCALAR one-point loops (a=0, identity vertex; W = mult_Ylm_real, NO mult_sigma), FURNISHED (Eq. 1.55):
//   J_1   = tr[W_lm(t) D_m^{-1}]           -> h0/disc/ylm/s0/l{l}/m{m}/J       (from phi = D_m^{-1} eta)
//   J_1mD = tr[W_lm(t) (1-D_ov) D_m^{-1}]  -> h0/disc/ylm/s0_1mD/l{l}/m{m}/J   (from dphi = (1-D_ov) phi)
// Both COMPLEX.  Analysis: sigma_PS = 2 Re J_1 ; sigma_FS = J_1 - conj(J_1mD) ; C_d = two_point(J).
// The (1-D_ov) is a cheap mat-vec on phi -- NO extra solve.  --IsScalarOnly computes ONLY the s0/s0_1mD loops
// and APPENDS them into the existing disc .h5 (skip J^a/condensate).
//
...
void PrintHelp(){
  ...
  printf("  --IsScalarOnly       compute ONLY the h0/disc/ylm/s0 loop and APPEND it into the existing disc\n");
  printf("                       per-hit .h5 (skip vector J^a + condensate).  OMIT => compute all fresh.\n");
  printf("  -h, --help\n");
  exit(0);
}
...
void ParseArgs(int argc, char* argv[],
               ...
               int& kmin, int& kmax, bool& is_scalar_only){
...
int main(int argc, char* argv[]){
  ...
  // GW factor (1-D_ov) on the sigma_FS one-point loop J_1mD = tr[W (1-D_ov) D_m^{-1}].
  ...
  // massless D_ov apply: op_oneMinusD : v -> (1 - D_ov) v.
  ...
  FermionVector eta, eta_A, tmp, phi, dphi, Gamma_phi;   // dphi = (1-D_ov) phi (FS one-point loop J_1mD)
  ...
  for(int k = k_lo; k < k_hi; k += k_ckpoint){
    ...
    for(int h=0; h<nhits; h++){
      ...
      // Gating.  Default: skip if "complete".  IsScalarOnly: skip if the s0 loop is already present; else
      // APPEND into the existing disc file (append_mode) or write a fresh scalar-only file if none exists.
      bool append_mode = false;
      if(std::filesystem::exists(h5path_h)){
        bool has_complete=false, has_s0=false;
        try {
      ...
      // SCALAR a=0 one-point loops (COMPLEX): JS = J_1 = tr[W_lm D_m^{-1}] (identity);
      // JS1mD = J_1mD = tr[W_lm (1-D_ov) D_m^{-1}] (GW).  Analysis: sigma_PS=2Re J_1, sigma_FS=J_1 - conj(J_1mD).
      std::vector<std::vector<std::vector<Complex>>> JS(L_MAX_YLM+1), JS1mD(L_MAX_YLM+1);
      for(int l=0;l<=L_MAX_YLM;l++){ JS[l].assign(2*l+1, std::vector<Complex>(Nt, Complex(0,0)));
      ...
      for(int t_s=0; t_s<interval; t_s++){
        for(int spin=0; spin<NS; spin++){
          ...
          // SCALAR loops (a=0, identity vertex; NO mult_sigma).  Always run.
          //   J_1   = tr[W_lm D_m^{-1}]         (from phi)
          //   J_1mD = tr[W_lm (1-D_ov) D_m^{-1}] (from dphi = (1-D_ov) phi)
          op_oneMinusD.from_cpu<N>(dphi.field, phi.field);   // (1-D_ov) phi
          for(int l=0; l<=L_MAX_YLM; l++){
            for(int m=-l; m<=l; m++){
              Gamma_phi = phi;
              Gamma_phi.mult_Ylm_real(l, m, base);
              eta.accumulate_loop_raw(JS[l][m+l], Gamma_phi, t_s, disc_tblock, spin);
              Gamma_phi = dphi;
              Gamma_phi.mult_Ylm_real(l, m, base);
              eta.accumulate_loop_raw(JS1mD[l][m+l], Gamma_phi, t_s, disc_tblock, spin);
            }
          }
        }
      }

      // ---- write ----  append_mode: copy the existing disc file, add ONLY the s0 loop, atomic rename back.
      ...
      // SCALAR a=0 complex one-point loops (always written): s0 = J_1 = tr[W D_m^{-1}] ;
      // s0_1mD = J_1mD = tr[W (1-D_ov) D_m^{-1}].  Analysis: sigma_PS=2Re J_1, sigma_FS=J_1 - conj(J_1mD).
      for(int l=0; l<=L_MAX_YLM; l++){
        for(int m=-l; m<=l; m++){
          const std::string kp =hp+"disc/ylm/s0/l"    +std::to_string(l)+"/m"+std::to_string(m)+"/J";
          const std::string kpd=hp+"disc/ylm/s0_1mD/l"+std::to_string(l)+"/m"+std::to_string(m)+"/J";
          write_vec(h5, kp,  JS[l][m+l]);
          write_vec(h5, kpd, JS1mD[l][m+l]);
        }
      }
      ...
      const char* mode = is_scalar_only ? (append_mode?"scalar-only APPEND (s0)":"scalar-only fresh (s0)")
                                        : "disc ylm s1/s2/s3 + sigma_PS + s0";
