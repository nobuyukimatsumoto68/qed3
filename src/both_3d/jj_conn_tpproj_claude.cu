...
int main(int argc, char* argv[]){
  ...
  const std::string dir_out = "data_"+esnid+"/tp_"+current
                            +"_nt0"+std::to_string(n_t0)+"_nhits"+std::to_string(nhits)+"/";
  ...
  // ---- host buffers (Sec. 3.2 symbols) ----------
  //   phi := phi' (shared forward leg),  psi := source leg (per site/origin),
  //   phit := looped current,  rho := K-applied source,  tmp := preconditioned CG RHS.
  ...
  for(int k = 0; k <= kmax; k += k_ckpoint){
    ...
    h5.createDataSet("t0s",   t0s);                       // the n_t0 source origins
    ...
    for(int h=0; h<nhits; h++){
      ...
      // (++) channel: phi'_++ = D_m^{-1} eta (forward solve; shared across origins/sites)
      ...
      for(int b=0; b<n_t0; b++){
        ...
        for(int n=0; n<n_sites; n++){
          // psi_++ = D_m^{-dag} K^dag(n,t0) eta  (dagger solve)
          ...
          // full dt: phi(n,t0+dt) = K(n,(t0+dt)%Nt) phi'_++;  Cpp[dt] += w_n psi^dag phi
          for(int dt=0; dt<Nt; dt++){
            kop.set_temporal(U, (t0+dt)%Nt, (Idx)n, /*dag=*/false);
            op_K.from_cpu<N>(phit.field, phi.field);
            Cpp[dt] += w_tp[n] * psi.dag(phit);
          }
        }
      ...
      if(parity){
        ...
        for(int b=0; b<n_t0; b++){
          ...
          for(int n=0; n<n_sites; n++){
            // psi_-- = tilde^{-1} K(n,t0) eta  (forward solve: adj RHS-former op_tilDmH; K, not K^dag)
