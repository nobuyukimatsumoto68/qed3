...
struct U1WilsonExt {
  ...
  ActionExt & operator=(const ActionExt&) = delete;

  template <typename Gauge>
  double operator()( const Gauge& U ) const {
    double res = 0.0;
    // spatial
    for(int s=0; s<U.lattice.Nt; s++){
      int i=0;
      for(const Face& face : U.lattice.base.faces) {
        if constexpr(U.is_compact)
          res += - 1.0/U.lattice.base.vps[i] *  std::cos( U.plaquette_angle(s, face) );
        else res += 0.5/U.lattice.base.vps[i] * std::pow( U.plaquette_angle(s, face), 2 );
        i++;
      }
    }
    // temporal
    for(int s=0; s<U.lattice.Nt; s++){
      // int i=0;
      for(const Link& link : U.lattice.base.links) {
        if constexpr(U.is_compact)
          res += - std::cos( U.plaquette_angle(s, link) );
        else res += std::pow( U.plaquette_angle(s, link), 2 );
        // i++;
      }
    }
    res *= beta;
