#!/usr/bin/env bash
# run_disc_stride5_L2_claude.sh -- STRIDE=5 disconnected jj Y_lm tower, nhits=2, L=2 ONLY.
# Sparser sibling of run_disc_stride2_L2_claude.sh: the stride-5 grid is ~2.5x LESS work than
# stride-2, so L2 disc finishes sooner (freeing the GPUs for other jobs).  a_t is auto-derived
# from each ensemble dir now (at0.1 -> a_t=0.1, at0.2 -> a_t=0.2), no --at flag needed.
#
...
# To leave a GPU free for another job, launch with e.g.  GPUS="0 0" bash run_disc_stride5_L2_claude.sh
# (2 workers on GPU0 only); default GPUS="0 0 1 1" uses both GPUs.
#
# RESUMABLE / SAFE: each (config,hit) is its own file corr.<k>.h<h>.h5 gated on "complete", RNG seed is
# (esnid,k,h) independent of stride/nhits, so re-running skips finished work.  Any stride-5 config that
# the stride-2 grid already computed (shared k, e.g. multiples of 10) stays bit-identical and is skipped.
# NO rm and NO kill anywhere (per project policy).
#
...
#   nohup bash run_disc_stride5_L2_claude.sh > run_disc_stride5_L2_claude.log 2>&1 &
# Per-job logs: disc_s5_L2_*_w{0..3}_claude.log (this directory).
set -u
...
echo "### START stride-5 L2 disc: ${#GPUS[@]} workers (GPUs ${GPUS[*]}), MPS  [$(date +%F_%H:%M:%S)] ###"
for (( W=0; W<${#GPUS[@]}; W++ ))
do
  worker "$W" &
done
wait
echo "### ALL stride-5 L2 disc done  [$(date +%F_%H:%M:%S)] ###"
