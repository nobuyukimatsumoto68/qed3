#!/usr/bin/env bash
# run_disc_stride5_L1_claude.sh -- STRIDE=5 disconnected jj Y_lm tower, nhits=2, L=1, gsq=1.5 ONLY.
# Boosts the L1 gsq=1.5 (strong-coupling, a=1.5 trajectory) disc statistics from the stride-10 grid
# (~398 matched cfg) toward the stride-5 grid (~2x), for the disc-limited L1 g1.5 vector.
#
# PACKING: 4 workers = 2 procs/GPU on 2 GPUs under MPS (L1 disc ~75% util -> packing pays ~1.33x, NM).
# Each worker does a config sub-stream (worker W starts at first+STRIDE*W, effective stride STRIDE*NW);
# the union of the 4 workers is the full stride-5 grid.
#
...
# stride-5 run FILLS the gaps (configs 25,35,... between the existing 20,30,40,...).
#
...
#   nohup bash run_disc_stride5_L1_claude.sh > run_disc_stride5_L1_claude.log 2>&1 &
# Per-job logs: disc_s5_L1_*_w{0..3}_claude.log (this directory).
set -u
...
DISC_TB=2
TARGET_L=1
GSQ_FILTER="${GSQ_FILTER:-1.500000}"     # only ensembles with this gsq (set '' to disable filtering)
...
# ---- L1 binary (rebuild when .cu/.h newer; FORCE_BUILD=1 to force) ----
need_build () {
...
# ---- collect L1 gsq=1.5 massless ensembles ----
ENS=()
...
do
  ...
  if [ -n "$GSQ_FILTER" ]
  then
    [ "$(get_gsq "$d")" = "$GSQ_FILTER" ] || continue
  fi
  ENS+=("$d")
done
echo "### STRIDE=$STRIDE  NHITS=$NHITS  L=$TARGET_L  gsq=${GSQ_FILTER:-ALL}  tb=$DISC_TB ###"
echo "### ${#ENS[@]} ensembles, ${#GPUS[@]} workers (GPUs ${GPUS[*]}), MPS packed ###"
...
run_disc () {   # $1=ens-dir  $2=GPU  $3=WID  $4=NWORKERS
  ...
  LOG="disc_s5_L${L}_g${gsq}_Nf${nf}_w${WID}_claude.log"
  ...
  {
    echo "### DISC-s5 $ens  L${L}  GPU${GPU}  w${WID}  kmin=$kmin_w stride=$stride_eff kmax=$kmax tb=$DISC_TB nhits=$NHITS (~$nconf cfg)  [$(date +%F_%H:%M:%S)] ###"
    ...
    echo "### DISC-s5 $ens w${WID} done (status $?)  [$(date +%F_%H:%M:%S)] ###"
  } >> "$LOG" 2>&1
}
...
if [ "${#ENS[@]}" -eq 0 ]
then
  echo "### no matching ensembles found -- nothing to do ###"
  exit 0
fi
echo "### START stride-5 L1 gsq=$GSQ_FILTER disc: ${#GPUS[@]} workers (GPUs ${GPUS[*]}), MPS  [$(date +%F_%H:%M:%S)] ###"
for (( W=0; W<${#GPUS[@]}; W++ ))
do
  worker "$W" &
done
wait
echo "### ALL stride-5 L1 gsq=$GSQ_FILTER disc done  [$(date +%F_%H:%M:%S)] ###"
