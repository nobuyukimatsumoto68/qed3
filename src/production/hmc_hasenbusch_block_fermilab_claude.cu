// hmc_hasenbusch_block_claude.cu
...
// _fermilab: ABSOLUTE geometry paths (FNAL), mimicking the src/both_3d fermilab drivers
// (hmc_fermilab_wmass_*_claude.cu all use this exact tree). The stock hmc_hasenbusch_block_claude.cu uses
// the LOCAL relative "../../geometry/..." convention; on FNAL the run CWD is the /lustre2 output dir, so
// relative resolution breaks -- hardcode the absolute FNAL geometry tree instead.
