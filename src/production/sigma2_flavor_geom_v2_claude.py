#!/usr/bin/env python3
# sigma2_flavor_geom_v2_claude.py  [chunk 2: a fixed FLAVOR on the full geometry basis {s2_00,O_2m,O_1m}]
#   Builds the 3x3 GEOMETRY matrix for ONE flavor (PP/FF/FP) -- e.g. FP = sigma_FS sigma_PS ("PS-FS") on
#   {sigma^2_00, O_2m (antipodal), O_1m (coincident split)} -- then the PRODUCTION Hankel+rebase GEVP
#   (Dt=OFFSETS, rebase NKEEP@REBT, metric T0), exactly as fs_channels_v2_hankel.
#   Flavor factor per perm: FLAVFAC[ip] = prod over cycles c of (1 + (-1)^{ sum_{v in c} fsmask[v] }),
#   fsmask over 4 vertices (0,1 sink geom ga ; 2,3 source geom gb) with the flavor's [FS,PS] pattern on each op.
#   Run: FLAVOR=FP OFFSETS=0,2,4 REBT=4 NKEEP=2 T0=3 BINSIZE=10 NPROC=12 python3 sigma2_flavor_geom_v2_claude.py
...
FLAVOR = os.environ.get("FLAVOR", "FP")
...
OFF_GEOM = [(0, 0), (0, 0), (0, SPLIT)]            # geometry time offsets: s2_00, O_2m, O_1m
FLAV = {"PP": [0, 0], "FF": [1, 1], "FP": [1, 0]}  # fsmask per op (1=FS)
GEOLAB = ["s2_00", "O_2m", "O_1m"]
...
FFAC = flavfac(FLAV[FLAVOR] + FLAV[FLAVOR])         # same flavor at sink and source
    ...
    for dt in range(DTMAX):
        ...
        for ga in range(3):
            for gb in range(3):
                ...
                C[ga, gb, dt] = (FFAC @ base).real / len(s0s)
    return C
...
def main():
    ...
    cache = "%s/sigma2_flavorgeom_%s_%dcfg_%s_d%d_claude.npy" % (CACHEDIR, tag.replace(".", "p"), len(ks), FLAVOR, SPLIT)
    if os.path.exists(cache):
        allC = np.load(cache)
        print("# loaded <- %s" % cache)
    else:
        print("# building FLAVOR=%s 3-geom matrix, %d cfg, %d workers ..." % (FLAVOR, len(ks), NPROC))
        if NPROC > 1:
    ...
    tmax = em_c.shape[0]
    print("\n# FLAVOR=%s {s2_00,O_2m,O_1m} Hankel+rebase  Dt=%s reb%d@%d T0=%d  %d cfg" % (FLAVOR, OFFSETS, NKEEP, REBT, T0, ncfg))
    print("#  t |  " + "  ".join("m%d(err)   " % n for n in range(NKEEP)) + "  [ref 0.46 / 0.62]")
    ...
    ax.set_title(r"FLAVOR=%s {$\sigma^2_{00},O_{2m},O_{1m}$} Hankel+rebase Dt=%s reb%d@%d  %s L1 %dcfg"
                 % (FLAVOR, OFFSETS, NKEEP, REBT, tag, ncfg), fontsize=10)
    ax.legend(fontsize=9, loc="upper right")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    os.makedirs("figs", exist_ok=True)
    out = "figs/sigma2_flavorgeom_%s_%s_claude.png" % (FLAVOR, tag)
