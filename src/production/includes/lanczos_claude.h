...
{
  constexpr Idx N = Comp::N;
  constexpr int orth_period = 1;   // FULL re-orthogonalization every step (_claude): partial (4) loses
                                   // orthogonality on (near-)degenerate clusters -> ghost Ritz values that
                                   // never hit eresid.  N is small enough that full re-orth is cheap.
