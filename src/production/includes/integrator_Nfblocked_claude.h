...
// integrator_Nfblocked_claude.h -- MinimumNorm2Block: a structural copy of MinimumNorm2
...
#ifdef BLOCK_FORCE
// MinimumNorm2BlockF: MinimumNorm2Block with the FORCE also blocked. Each fermion kick =
// bpf.update_eta() (block action solve) + bpf.compute_force(dSf,U) (ONE total force over all NSTACK
// flavors) -> a single pi kick. Gauge sweeps / lambda weights identical to MinimumNorm2(Block).
struct MinimumNorm2BlockF {
  const double stot;
  const int nsteps;
  const int nsteps_inner;
  ...
  MinimumNorm2BlockF( const double stot_=1.0, const int nsteps_=10, const int nsteps_inner_=10)
    : stot(stot_)
    , nsteps(nsteps_)
    , nsteps_inner(nsteps_inner_)
    , tau(stot/nsteps)
  ...
  void integrate( Gauge& U, Force& pi,
    ...
    for(int n=0; n<nsteps; n++) {
      ...
      // inner (1-2lambda) fermion kick : block action + block force
      bpf.update_eta();
      bpf.compute_force( dSf, U );
#ifdef InfoForce
      dSf.print2log_norm( "# Sf : " );
      ...
      // last (2*)lambda fermion kick : block action + block force
      bpf.update_eta();
      bpf.compute_force( dSf, U );
#ifdef InfoForce
      dSf.print2log_norm( "# Sf : " );
