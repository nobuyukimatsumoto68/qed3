...
// _claude: L4 FORCE-ONLY mixed-precision inner multishift (mixedprec_impl_plan_claude.md). Guarded so
// non-MIXED_FORCE builds are byte-identical. Routes ONLY the force precalc_grad_*_ms solve_multishift
// (NOT the action mult/adj_ms -- those stay fp64/exact) through the tuned MixedSolver.
#ifdef MIXED_FORCE
#include <memory>
#include "sparse_matrix_mixed_claude.h"
#include "matpoly_mixed_claude.h"
#endif
...
struct OverlapWMass : public Zolotarev {
  ...
  bool is_lambda_fixed = false;   // _claude: freeze lambda_max after the first update() (HMC-exactness fix, 2026-07-12)
...
#ifdef MIXED_FORCE
  std::unique_ptr<MixedSolver<N>> mixed_force = nullptr;  // lazy-built (needs frozen lambda_max)
#endif
  ...
  // _claude (Hasenbusch C2a, hasenbusch_massless_impl_plan_claude.md): cheap in-place mass switch so
  // ONE shared operator can serve every Hasenbusch frame. Only `mass`/`mass_coeff` change; M_mass
  // (the geometric diag A_y/Abar) is fixed, and update(U) (Wilson links + lambda_max) is
  // mass-INDEPENDENT -- so NO re-update / no sign-function recompute is needed on a mass switch.
  // Invalidates is_precalc (the massive force fold in precalc_grad_ms depends on mass_coeff).
  void set_mass( const Complex m ) {
    mass = m;
    mass_coeff = mass * (DW.lattice.mean_dual_area / DW.lattice.mean_ell); // m * Abar/abar_s (complex)
    is_precalc = false;
  }

  // _claude (Hasenbusch, per NM 2026-07-13): set mass_coeff DIRECTLY -- the auxiliary Hasenbusch mass is
  // the coefficient c multiplying M_mass, so m_L = c * M_mass = c * diag(A_y/Abar) (NOT the physical m,
  // which the operator would rescale by Abar/abar_s). `mass` is kept consistent (= c * abar_s/Abar) only
  // for bookkeeping; every downstream apply/force/action keys off mass_coeff. Frame 0 (c=0) = massless.
  void set_mass_coeff( const Complex c ) {
    mass_coeff = c;
    mass = c * (DW.lattice.mean_ell / DW.lattice.mean_dual_area);   // inverse of set_mass (bookkeeping only)
    is_precalc = false;
  }

  // _claude (HMC-exactness fix, 2026-07-12 per NM): the driver calls this after a few thermalization
  // trajectories to FREEZE lambda_max. Recomputing it every MD step made D_ov change step-to-step while
  // the force omits d lambda_max/dU -> the HMC was not the exact integrator of the (approximate) action
  // -> a step-independent dH floor (~delta at coarse L1). Policy: FREE (recompute) while thermalizing,
  // then freeze -> exact thereafter.
  void freeze_lambda(){ is_lambda_fixed = true; }
  void unfreeze_lambda(){ is_lambda_fixed = false; }   // _claude: re-enable compute_lambda_max on the next update() (used to re-freeze per config during tuning)

  // _claude (per NM 2026-07-13): FIX the Zolotarev window to hardcoded (lambda_min, lambda_max), chosen
  // from the Wilson low-mode distribution (eig_wilson_lowmode scan), and FREEZE. The sign approx is rebuilt
  // on k = lambda_min/lambda_max (n poles) and held constant -- no per-config recompute. This supersedes
  // freeze-at-thermalized for production: one fixed, ensemble-covering window -> exact + reproducible.
  // See frozen_window_claude.h for the per-L values. Call once after construction, before the first update().
  void set_lambda( const double lmin, const double lmax ){
    lambda_min = lmin;
    lambda_max = lmax;
    Zolotarev::update( lmin/lmax );   // rebuild the rational approx on the fixed window k = lmin/lmax
    is_lambda_fixed = true;           // hold both edges -> update() never recomputes
  }

  template<typename Gauge>
  void update( const Gauge& U ) {
    d_DW.update( U );
    // compute_lambda_max();   // OLD: per-step recompute (broke exactness)
    if( !is_lambda_fixed ) compute_lambda_max();   // free while thermalizing; held once freeze_lambda() called
...
#pragma omp parallel for num_threads(nstreams)
  ...
  // _claude: FORCE inner multishift (A=(1/lambda_max^2)D_W^dag D_W, shifts -k^2/cp[m]) -> d_blk. Under
  // MIXED_FORCE routes through the tuned MixedSolver (fp32 pass + fp64 tail + per-shift cleanup, still
  // converges to Comp::TOL_INNER=1e-9), else the fp64 MatPoly::solve_multishift (byte-identical). Call
  // ensure_mixed_force() ONCE per force eval first (rebuilds the fp32 CSR mirror from the current U).
#ifdef MIXED_FORCE
  void ensure_mixed_force() {
    if(!mixed_force) mixed_force.reset(new MixedSolver<N>(M_DW, M_DWH, 1.0/(lambda_max*lambda_max), d_DW.len));
    else             mixed_force->refresh();   // re-cast fp32 val from the updated fp64 D_W (per MD step)
  }
#else
  void ensure_mixed_force() {}
#endif

  void solve_force_ms(CuC* d_blk, const CuC* d_rhs) {
    std::vector<double> sigma(size-1);
    for(int m=1; m<size; m++) sigma[m-1] = -k*k/cp[m];
#ifdef MIXED_FORCE
    mixed_force->solve_multishift(d_blk, d_rhs, sigma.data(), size-1, Comp::TOL_INNER);
#else
    MatPoly Aseed;
    Aseed.push_back( cplx(1.0/(lambda_max*lambda_max)), {&M_DW, &M_DWH} );
    Aseed.solve_multishift<N>( d_blk, d_rhs, sigma.data(), size-1, Comp::TOL_INNER );
#endif
  }
  ...
  void precalc_grad_deviceAsyncLaunch_ms( const Gauge& U, const CuC* d_eta ) {
    ...
    ensure_mixed_force();   // MIXED_FORCE: rebuild the fp32 CSR mirror for this U; no-op otherwise

    solve_force_ms( d_Zblock, d_eta );                                                     // Z_m = R_m eta
    for(int m=1; m<size; m++) CUDA_CHECK(cudaMemcpy(d_Zs[m], d_Zblock + (size_t)(m-1)*N, N*CD, D2D));

    solve_force_ms( d_Yblock, d_Ys[0] );                                                   // Y_m = R_m (X^dag eta)
    ...
    if( mass != Complex(0.0,0.0) ){
      ...
      solve_force_ms( d_Yblock, d_mtmp );                                                  // W_m = R_m X^dag(M_mass eta)
      for(int m=1; m<size; m++)
...
#pragma omp parallel for num_threads(nstreams)
  ...
  void precalc_grad_bilinear_deviceAsyncLaunch_ms( const Gauge& U, const CuC* d_bra, const CuC* d_ket ) {
    ...
    solve_force_ms( d_Zblock, d_ket );                                                     // Z_m = R_m (ket=eta_i)
    for(int m=1; m<size; m++) CUDA_CHECK(cudaMemcpy(d_Zs[m], d_Zblock + (size_t)(m-1)*N, N*CD, D2D));

    solve_force_ms( d_Yblock, d_Ys[0] );                                                   // Y_m = R_m (X^dag bra)
    for(int m=1; m<size; m++) CUDA_CHECK(cudaMemcpy(d_Ys[m], d_Yblock + (size_t)(m-1)*N, N*CD, D2D));
