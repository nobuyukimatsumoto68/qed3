...
// pseudofermion_claude.h (C4b): copy of pseudofermion.h that drives the overlap through
// the multi-shift path -- the heatbath uses adj_deviceAsyncLaunch_ms and the action CG
// uses LinOpDHDWrapper from sparse_matrix_claude.h (DHD_deviceAsyncLaunch_ms). Include
// sparse_matrix_claude.h (not sparse_matrix.h) alongside this in the same .cu. The HMC
// FORCE still uses the pole-loop precalc_grad/grad (multi-shift force = a later chunk).
...
struct PseudoFermion {
  ...
  void gen( Rng& rng ) {
    ...
    // D.adj_deviceAsyncLaunch( d_phi, d_xi );      // original (pole loop)
    D.adj_deviceAsyncLaunch_ms( d_phi, d_xi );      // C4b: multi-shift
