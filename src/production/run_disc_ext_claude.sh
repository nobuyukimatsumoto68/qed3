#!/usr/bin/env bash
# run_disc_ext_claude.sh -- EXTENDED-STATISTICS disconnected jj Y_lm tower
# (jj_local_ylm_disc_stoch_claude.cu) on ALL massless redo ensembles (incl. L4).  STRIDE=10 (2x the
# old stride-20 grid), nhits 1, disc-tblock=2 (tb2).  NO thermalization cut at measurement (kmin =
...
#
# Run detached:
#   nohup bash run_disc_ext_claude.sh > run_disc_ext_claude.log 2>&1 &
# Per-job logs: disc_*_w{0,1}_claude.log (this directory) -- APPEND to the existing disc logs.
set -u
...
run_disc () {   # $1=ens-dir  $2=GPU  $3=WID
  ...
  {
    echo "### DISC-EXT $ens  L${L}  GPU${GPU}  w${WID}  kmin=$kmin_w stride=$stride_eff kmax=$kmax tb=$DISC_TB (~$nconf cfg)  [$(date +%F_%H:%M:%S)] ###"
...
echo "### START disc-ext: ${NWORKERS} workers (GPUs ${GPUS[*]}), STRIDE=$STRIDE, tb=$DISC_TB, NO MPS  [$(date +%F_%H:%M:%S)] ###"
for (( W=0; W<NWORKERS; W++ ))
do
  worker "$W" &
done
wait
echo "### ALL disc-ext done  [$(date +%F_%H:%M:%S)] ###"
