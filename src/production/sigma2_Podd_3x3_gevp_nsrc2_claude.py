#!/usr/bin/env python3
# sigma2_Podd_3x3_gevp_claude.py  [dedicated parity-odd P- 3x3 GEVP + labeled figure]
#   Parity-odd sector of the flavor x geometry basis: FP = sigma_FS sigma_PS on {sigma^2_00, O_2m,
#   O_1m} = 3 ops (flat indices flavor*3+geom, flavor 2 = FP -> OPS = 6,7,8).  FP is parity-odd in
#   the continuum; at finite a it overlaps the same 0++ tower (parity doubling: <FP|even>=0 exactly
#   via GW, so P- is a distinct-but-degenerate copy of the P+ spectrum).  Reads the FULL 9-op cache
#   from sigma2_flavorgeom_full_v2_claude.py, selects the P- block, runs the PRODUCTION block-Hankel
#   + staged-rebase GEVP (Dt=OFFSETS, rebase NKEEP@REBT, metric point T0).  Writes the labeled figure
#   figs/sigma2_Podd_3x3_reb<NKEEP>_T0<T0>_<tag>_claude.png (distinct from the P+ / full-9op figs).
#
#   T0 (metric point): timeslice whose C(T0) defines the GEVP metric; 0 <= T0 < REBT, C(T0)
#   well-conditioned (small T0; canonical 3).  Canonical P- was reb2@4 T0=3.
#
...
#   Run: OPS=6,7,8 OFFSETS=0,2,4 REBT=4 NKEEP=2 T0=3 BINSIZE=10 python3 sigma2_Podd_3x3_gevp_claude.py
...
OPS = [int(x) for x in os.environ.get("OPS", "6,7,8").split(",")]

FLABS = ["PP", "FF", "FP"]
GEOLAB = ["s2", "O2m", "O1m"]
OPLAB = ["%s-%s" % (f, g) for f in FLABS for g in GEOLAB]    # flavor*3+geom
STATE_LAB = ["ground", "2-meson", "state 2", "state 3", "state 4"]
...
def main():
    ...
    import glob
    import re
    hits = glob.glob("%s/sigma2_flavorgeom_FULL_%s_L%d_*cfg_nsrc2_d%d_claude.npy" % (CACHEDIR, tag.replace(".", "p"), dc.L, SPLIT))
    if not hits and dc.L == 1:
        # L1 nsrc2 cache predates the _L%d naming (no _L tag = L1 by convention); accept it, exclude other L.
        hits = [h for h in glob.glob("%s/sigma2_flavorgeom_FULL_%s_*cfg_nsrc2_d%d_claude.npy" % (CACHEDIR, tag.replace(".", "p"), SPLIT))
    ...
    cols = ["tab:green", "tab:red", "tab:orange", "gray", "tab:blue"]
    mkr = ["o", "s", "^", "x", "D"]
    ...
    ax.set_title(r"P- 3x3 FP$\times$geom reb%d@%d T0=%d  %s L%d %dcfg" % (NKEEP, REBT, T0, tag, dc.L, ncfg), fontsize=11)
    ax.legend(fontsize=9, loc="upper right")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    os.makedirs("figs", exist_ok=True)
    out = "figs/sigma2_Podd_3x3_reb%d_T0%d_%s_L%d_nsrc2_claude.png" % (NKEEP, T0, tag, dc.L)
