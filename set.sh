...
dir_qfe=${dir_include}/newQFE_copy
...
if [ ! -L ${dir_qfe} ] && [ ! -d ${dir_qfe} ]; then
    cd ${dir_include}
    echo "--- newQFE_copy not found"
    git clone https://github.com/nobuyukimatsumoto68/newQFE_copy.git
    echo "---  newQFE_copy installed"
fi

dir_eigen=${dir_include}/Eigen
if [ ! -L ${dir_eigen} ] && [ ! -d ${dir_eigen} ]; then
    ...
    cd ${dir_qfe}/include
    ln -s ${dir_include}/eigen_src/Eigen Eigen
    echo "--- Eigen installed"
fi

dir_boost=${dir_include}/boost
if [ ! -L ${dir_boost} ] && [ ! -d ${dir_boost} ]; then
    ...
    ./b2 headers
    cd ..
    ln -s boost_1_89_0/boost boost
    cd ${dir_qfe}/include
    ln -s ${dir_include}/boost_1_89_0/boost boost
